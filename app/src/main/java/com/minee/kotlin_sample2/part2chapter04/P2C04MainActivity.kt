...
class P2C04MainActivity : AppCompatActivity() {
    ...
    lateinit var db: P2C04Database

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_p2_c04_main)

        db = Room.databaseBuilder(
            applicationContext,
            P2C04Database::class.java,
            "historyDB"
        ).build()
    }
    ...
    private fun numberButtonClicked(number: String) {
        ...
        if (expressionText.isNotEmpty() && expressionText.last().length >= 15) {
            Toast.makeText(this, "숫자는 15자리 까지만 입력 가능합니다", Toast.LENGTH_SHORT).show()
            return
    ...
    fun resultButtonClicked(v: View) {
        ...
        if (expressionTexts[0].isNumber().not() || expressionTexts[2].isNumber().not()) {
            Toast.makeText(this, "입력값 오류입니다", Toast.LENGTH_SHORT).show()
            return
        }
            ...
            db.historyDao().getAll().reversed().forEach {
                runOnUiThread {
                    val historyView = LayoutInflater.from(this)
                        .inflate(R.layout.activity_p2_c04_history_row, null, false)
